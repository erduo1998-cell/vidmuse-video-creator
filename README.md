...
真实生成时使用了两部分：四格参考图，以及同一次提交里写明时间、画面、运动和声音的提示词。下面把两部分合成一张完整执行故事板，所有标注都来自当时的真实生成记录，不是事后补编。

**1. 实际生成执行故事板**

![包含时间、画面、运动和声音标注的真实生成故事板](docs/demos/vidmuse-h3-storyboard-annotated.png)

[查看当时原样提交的四格参考图](docs/demos/vidmuse-h3-storyboard.png)
